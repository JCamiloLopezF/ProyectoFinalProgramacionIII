...
LuxoraWallet luxora = LuxoraWallet.getInstanciaUnica();
    ...
    Usuario usuarioSeleccionado = luxora.getUsuarioSeleccionado().get(0);
    GestorArchivo gestor = new GestorArchivo();

    @FXML
    void btn_aniadirCuenta(ActionEvent event) throws IOException {
        ...
        String tipoCuenta;
        try {
            tipoCuenta = tipoCuentaStr.toUpperCase();
        ...
        Cuenta cuenta = new Cuenta(idCuenta, nombreBanco, numeroCuenta, tipoCuenta, saldo, usuarioSeleccionado.getIdUsuario());

        gestorCuentas.agregarCuentaBancaria(usuarioSeleccionado, idCuenta, nombreBanco, numeroCuenta, tipoCuenta, saldo, usuarioSeleccionado.getIdUsuario());
        gestor.guardarCuentaBancaria(cuenta);
